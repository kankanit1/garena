...
import ItemDropdown from "../components/Dropdowns/itemDropdown";
import Image from "next/image";
import axios from "axios";
import Error from "next/error";

const Item = ({ loaderImg, item, error }) => {
  if (error) {
    return (
      <>
    ...
    <>
      <Layout>
        <div className="page-header item-header py-5">
          <h1 className="text-center bold page-header-title w-100">Items</h1>
        </div>
                ...
                <div className="col-lg-10 col-md-10 col-12">
                  ...
                  <div className="w-100 py-3">
                    <ul className="news-list">
                      {item.map((items) => (
                        <li key={items.id} className="news-list-row">
                          <Link
                            href={{
                              pathname: "/itemsdetail",
                              query: { id: items.id, userId: items.id },
                            }}
                            as={"/items/" + items.id}
                          >
                            <a className="card">
